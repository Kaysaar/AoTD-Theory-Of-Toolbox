...
public class StarSystemHoldingButton extends CustomButton {
    ...
    public void createContainerContent(CustomPanelAPI container) {
        ...
        if (buttonData instanceof StarSystemAPI system) {
            ...
            if(star!=null){
                component = new EntityWithNameComponent(star, true, StarSystemHoldingTable.widthMap.get("name"), 50);
            }
            else{
                component = new EntityWithNameComponent(center, true, StarSystemHoldingTable.widthMap.get("name"), 50);
            ...
            buttonWithImageComponent = new ButtonWithImageComponent(height - 4, height - 4, market.getAdmin().getPortraitSprite()) {
                ...
                public void performActionOnClickWithEvent(boolean isRightClick, InputEventAPI event) {
                    ...
                    ArrayList<Object> rows = (ArrayList<Object>) ReflectionUtilis.invokeMethodWithAutoProjection("getRows", ReflectionUtilis.getChildrenCopy((UIPanelAPI) origPanel).get(0));
                    for (Object row : rows) {
                        MarketAPI marketFound = (MarketAPI) ReflectionUtilis.findFieldByType(ReflectionUtilis.invokeMethodWithAutoProjection("getData", row), MarketAPI.class);
