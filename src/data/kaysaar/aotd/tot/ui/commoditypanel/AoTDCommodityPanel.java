...
public class AoTDCommodityPanel implements ExtendedUIPanelPlugin {
    ...
    public AoTDCommodityPanel(float width, float height, MarketAPI market, boolean isInDialog, AoTDDetailedCommodityPanelContent parent){
        this.market = market;
        this.mainPanel = Global.getSettings().createCustom(width,height,this);
        this.isInDialog = isInDialog;
        this.parent = parent;
    ...
    public void createUI() {
        ...
        Collections.sort(commodities,new Comparator<AoTDCommodityOnMarket>() {
            @Override
        ...
        TooltipMakerAPI tooltipHeader = contentPanel.createUIElement(contentPanel.getPosition().getWidth(),40,false);
        ...
        if(table!=null){
            ...
            table = new AoTDCommodityUITable(contentPanel.getPosition().getWidth(),contentPanel.getPosition().getHeight()-minusHeight,true,0,0,commodities,market,isInDialog,parent);
            table.createSections();
            table.createTable();
        }
        if(isInDialog){
            tooltipHeader.addSectionHeading(market.getName()+" Commodity Data", market.getFaction().getBaseUIColor(),market.getFaction().getDarkUIColor(),Alignment.MID,0f);

        }
        else{
            shortageButton = tooltipHeader.addAreaCheckbox("Use stockpiles during shortage",null,market.getFaction().getBaseUIColor(),market.getFaction().getDarkUIColor(),market.getFaction().getBrightUIColor(),contentPanel.getPosition().getWidth(),20,0f);
            shortageButton.setEnabled(market.isPlayerOwned()||market.getFaction().isPlayerFaction()||Global.getSettings().isDevMode());
            shortageButton.getPosition().inTL(0,0);
            if(market.isUseStockpilesForShortages()){
                shortageButton.highlight();
            }
            tooltipHeader.addSectionHeading("Market Commodity Data", market.getFaction().getBaseUIColor(),market.getFaction().getDarkUIColor(),Alignment.MID,5f).getPosition().inTL(0,23);
    ...
    public void renderBelow(float alphaMult) {
        if(!isInDialog){
            blackBG.setSize(mainPanel.getPosition().getWidth(), mainPanel.getPosition().getHeight());
            blackBG.setColor(Color.black);
            blackBG.setAlphaMult(alphaMult*0.7f);
            blackBG.renderAtCenter(mainPanel.getPosition().getCenterX(), mainPanel.getPosition().getCenterY());
        }
        ...
        if(shortageButton!=null){
            if(shortageButton.isChecked()){
                ...
                if(mainColonyPanel!=null){
                    ReflectionUtilis.invokeMethodWithAutoProjection("recreateWithEconUpdate",mainColonyPanel);
                }
            }
        }
    }
