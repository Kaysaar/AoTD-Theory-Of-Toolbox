...
public class AoTDCommodityUITable extends UITableImpl {
    ...
    public AoTDCommodityUITable(float width, float height, boolean doesHaveScroller, float xCord, float yCord, ArrayList<AoTDCommodityOnMarket> commodities, MarketAPI market, boolean isInDialog, AoTDDetailedCommodityPanelContent parent) {
        ...
        if(dropDownButtons.isEmpty()){
            for (AoTDCommodityOnMarket commodity : commodities) {
                AoTDCommodityTableDropDownButton bt = new AoTDCommodityTableDropDownButton(this,width-1,40,0,0,false,commodity);
    ...
    public void advance(float amount) {
        ...
        handleSortButton(buttonDemand,Comparator.comparing(x->x.commodity.getSupplyDemandData().getTotalRawUnitsFromDemand()));
        handleSortButton(buttonImport,Comparator.comparing(x->{
        ...
        handleSortButton(buttonExport,Comparator.comparing(x->{
    ...
    public void createSections() {
        ...
        buttonProd = tooltipOfButtons.addAreaCheckbox("Supply", SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("production"), 20, 0f);
        ...
        buttonImport = tooltipOfButtons.addAreaCheckbox("Exp / Imp", SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("import"), 20, 0f);
        buttonExport = tooltipOfButtons.addAreaCheckbox("Exc / Def", SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("deficit"), 20, 0f);
        buttonCommodity.getPosition().inTL(1, 0);
        buttonProd.getPosition().inTL(buttonCommodity.getPosition().getWidth() + 1, 0);
        float x = buttonCommodity.getPosition().getWidth() + 2 + buttonProd.getPosition().getWidth() + 1;
