...
public class AoTDCommodityInfoButton extends CustomButton {
    public AoTDCommodityInfoButton(float width, float height, Object buttonData, float indent, Color base, Color bg, Color bright) {
        ...
        if(buttonData instanceof AoTDCommodityOnMarket commodity){
            ...
            ImageViewer viewer = new ImageViewer(iconSize,iconSize,commodity.getCommoditySpec().getIconName()){
                ...
                public void render(float alphaMult) {
                    ...
                    this.spriteOfImage.setSize(this.componentPanel.getPosition().getWidth()*0.9f, this.componentPanel.getPosition().getHeight()*0.9f);
                    this.spriteOfImage.renderAtCenter(this.componentPanel.getPosition().getCenterX(), this.componentPanel.getPosition().getCenterY());
                }
            };
            ...
            production = container.createUIElement(AoTDCommodityUITable.widthMap.get("production"),container.getPosition().getHeight(),false);
            demand = container.createUIElement(AoTDCommodityUITable.widthMap.get("demand"),container.getPosition().getHeight(),false);
            imports = container.createUIElement(AoTDCommodityUITable.widthMap.get("import"),container.getPosition().getHeight(),false);
            exports = container.createUIElement(AoTDCommodityUITable.widthMap.get("deficit"),container.getPosition().getHeight(),false);
            ...
            production.addPara(AoTDToolboxMisc.getDGSStringWithSign(prod),prodC,0f).setAlignment(Alignment.MID);
            ...
            if(rawExp>0){
                ...
                ImageViewer viewer1 = new ImageViewer(iconSize-10,iconSize-10,Global.getSettings().getSpriteName("commodity_markers","exports"));
                container.addComponent(viewer1.getComponentPanel()).leftOfMid(viewer.getComponentPanel(),13);
            }
            else{
                expC = Color.ORANGE;
                ...
                imports.addPara(Misc.getWithDGS(rawExp),expC,0f).setAlignment(Alignment.MID);
                ImageViewer viewer1 = new ImageViewer(iconSize-10,iconSize-10,Global.getSettings().getSpriteName("commodity_markers","imports"));
                container.addComponent(viewer1.getComponentPanel()).leftOfMid(viewer.getComponentPanel(),13);
            }
            if(excess>0){
                excDef= Misc.getPositiveHighlightColor();
                exports.addPara(AoTDToolboxMisc.getDGSStringWithSign(excess),excDef,0f).setAlignment(Alignment.MID);
            }
            if(deficit>0){
                excDef= Misc.getNegativeHighlightColor();
                exports.addPara(AoTDToolboxMisc.getDGSStringWithSign(-deficit),excDef,0f).setAlignment(Alignment.MID);
            }
            ...
            container.addUIElement(production).inTL(AoTDCommodityUITable.getStartingX("production")+5,container.getPosition().getHeight()/2-10);
            container.addUIElement(demand).inTL(AoTDCommodityUITable.getStartingX("demand")+5,container.getPosition().getHeight()/2-10);
            container.addUIElement(imports).inTL(AoTDCommodityUITable.getStartingX("import")+5,container.getPosition().getHeight()/2-10);
            container.addUIElement(exports).inTL(AoTDCommodityUITable.getStartingX("deficit")+5,container.getPosition().getHeight()/2-10);
