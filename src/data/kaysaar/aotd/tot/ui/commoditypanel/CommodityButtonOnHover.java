...
public class CommodityButtonOnHover implements TooltipMakerAPI.TooltipCreator {
    ...
    public CommodityButtonOnHover(CommoditySpecAPI spec, MarketAPI market, boolean isInDialog,boolean incomeMode) {
        this.spec = spec;
        this.market = market;
        this.isInDialog = isInDialog;
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
        ...
        // ---------------- LOCAL ----------------
        tooltip.addSectionHeading("Local Production and Demand",
        ...
        if (supply > 0) {
            ...
            tooltip.addPara("There is no local production present on this market.", Misc.getGrayColor(), 3f);
            tooltip.addSpacer(10f);
        }
        ...
        if (demand > 0) {
            ...
            if (com.getDeficitQuantity() > 0) {
                ...
                if(Global.getSettings().isDevMode()){
                    tooltip.setBulletedListMode(BaseIntelPlugin.BULLET);
                    for (MutableStat.StatMod value : com.getExcDefData().deficit.getFlatMods().values()) {
                        tooltip.addPara("%s: %s",3f,new Color[]{Misc.getNegativeHighlightColor(),Misc.getTextColor()},Misc.getWithDGS(value.value),value.getDesc());
                    }
                    tooltip.setBulletedListMode(null);
                }
            }
            ...
            tooltip.addPara("There is no local demand present on this market.", Misc.getGrayColor(), 3f);
            tooltip.addSpacer(5f);
        }

        // ---------------- IMPORTS ----------------
        if (com.doesImport()) {
            tooltip.addSectionHeading("Imports",
                    market.getFaction().getBaseUIColor(),
                    market.getFaction().getDarkUIColor(),
                    Alignment.MID, 5f);
            tooltip.addSpacer(5f);
            ...
            tooltip.addPara("%s requires around %s units of %s.",
                    0f, Color.ORANGE,
                    market.getName(),
                    Misc.getWithDGS(imports),
                    spec.getName());
            ...
            if (independentImports > 0) {
                tooltip.addPara("%s purchased from external markets.",
                        3f, Color.ORANGE,
                        Misc.getWithDGS(independentImports));
            }
        ...
        // ---------------- EXPORTS ----------------
        if (com.getSupplyDemandData().getExport(com) > 0 || com.getSupplyDemandData().getTotalRawUnitsFromSupply() > 0) {

            tooltip.addSectionHeading("Export and Local Supply",
                    market.getFaction().getBaseUIColor(),
                    market.getFaction().getDarkUIColor(),
                    Alignment.MID, 5f);
            tooltip.addSpacer(5f);
            ...
            if (totalProduced > 0) {
                if (localized > 0 && exports <= 0) {
                    tooltip.addPara("%s sells %s units of %s.",
                            0f, Color.ORANGE,
                            market.getName(),
            ...
            if(!com.isDemandLegal()||!com.isSupplyLegal()){
                tooltip.addPara("We earn no income from local trade, as well as from exports due to commodity being illegal!",Misc.getNegativeHighlightColor(),3f);
            }
            else{
                tooltip.setBulletedListMode(BaseIntelPlugin.BULLET);
                ...
                if (md != null) {
                    ...
                    // This must prepare predicted allocations for the current month if contracts weren't executed yet.
                    // Implement in manager: compute predicted allocations without mutating remainingNet.
                    mgr.ensurePredictionsUpToDate();

                    boolean hasActual = md.exportedByContract != null && !md.exportedByContract.isEmpty();
                    ...
                    if (source != null && !source.isEmpty()) {
                        for (Map.Entry<String, LinkedHashMap<String, Integer>> e : source.entrySet()) {
                ...
                // If your getExport() isn't aligned with your solver snapshot, consider replacing this with an explicit
                // "externalExported" field on AoTDMarketData computed during the month-end pipeline.
...
// EXTERNAL EXPORT (remaining after contracts)
                ...
                if(!incomeMode){
                    if (AoTDToolboxMisc.getSpeculatedExportIncomeFromContractsForUI(com) != 0) {
                        String title = usingPredicted ? "Trade contracts (estimated)" : "Trade contracts";
                        tooltip.addSectionHeading(title,
                                market.getFaction().getBaseUIColor(),
                                market.getFaction().getDarkUIColor(),
                                Alignment.MID, 5f);
                        tooltip.addSpacer(5f);
                        ...
                        for (Map.Entry<String, Integer> e : contractShipments.entrySet()) {
                            ...
                            AoTDTradeContract c = contracts != null ? contracts.get(contractId) : null;
                            ...
                            if (c != null) {
                                ...
                                AoTDTradeContract.TradeContractData line = c.getContractData().get(spec.getId());
                                if (line != null) cut = Math.max(0f, line.getCutFromBasePrice());

                                if (c.getFactionId() != null) {
                                    target = Global.getSector().getFaction(c.getFactionId()).getDisplayName();
                                } else {
                            ...
                            String verb = creditsDelta >= 0 ? "generating roughly" : "costing roughly";
                            String creditsText = Misc.getDGSCredits(Math.abs((float) creditsDelta));

                            // Optional: show (player-issued) marker
                            String extra = (playerIssued ? " (Issued by "+Global.getSector().getPlayerPerson().getNameString()+")" : "");
