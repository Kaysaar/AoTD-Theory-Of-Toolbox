...
public class WarehouseSectionUI implements ExtendedUIPanelPlugin {
    ...
    public void createUI() {
        ...
        if (restWidth > WarehouseHoldingTable.getWidth()) {
            WarehouseHoldingTable.reDestributeAdditionalWidth(restWidth - WarehouseHoldingTable.getWidth());
        }
        if (warehouseHoldingTable == null) {
            warehouseHoldingTable = new WarehouseHoldingTable(WarehouseHoldingTable.getWidth() + 13, contentPanel.getPosition().getHeight(), true, 0, 0);
            warehouseHoldingTable.createSections();
            warehouseHoldingTable.createTable();
        }
        if (detailUI == null) {
            detailUI = new WarehouseDetailUI(detailedSectionWidth - 10, contentPanel.getPosition().getHeight());
        }
        contentPanel.addComponent(warehouseHoldingTable.mainPanel).inTL(0, 0);
        contentPanel.addComponent(detailUI.getMainPanel()).inTL(contentPanel.getPosition().getWidth() - 455, 0);
