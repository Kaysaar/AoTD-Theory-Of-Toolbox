...
public class WarehouseCustomButton extends CustomButton {
    ...
    public WarehouseCustomButton(float width, float height, Object buttonData, float indent, Color base, Color bg, Color bright) {
    ...
    public void createButtonContent(TooltipMakerAPI tooltip) {
        ...
        if(isWithArrow){
            ...
            tooltip.addCustom(panelIndicator,0f).getPosition().inTL((float) WarehouseHoldingTable.widthMap.get("name")*0.75f,centerY-7);
    ...
    public void createContainerContent(CustomPanelAPI container) {
        ...
        EntityWithNameComponent component = new EntityWithNameComponent(market.getPrimaryEntity(), WarehouseHoldingTable.widthMap.get("name"), 50, false);
        component.createUI();
        ...
        LocationPointerComponent component1 = new LocationPointerComponent(market,WarehouseHoldingTable.widthMap.get("location"),height,c);
        ...
        StorageItemsWidget widgetRes = new StorageItemsWidget((available/2)-5,height,market, StorageItemsWidget.Mode.COMMODITIES);
        StorageItemsWidget widgetAI = new StorageItemsWidget(100-5,height,market, StorageItemsWidget.Mode.AI_CORES);
        StorageItemsWidget widgetItems = new StorageItemsWidget((available/2)-5,height,market, StorageItemsWidget.Mode.SPECIAL_ITEMS);
        ...
        if(!plugin.getCargo().getMothballedShips().getMembersListCopy().isEmpty()){
            ImageViewer viewer = new ImageViewer(20,20,Global.getSettings().getCommoditySpec(Commodities.SHIPS).getIconName());
            container.addComponent(viewer.getComponentPanel()).rightOfMid(reference,10);
            tooltipIncome.addTooltipTo(new TooltipMakerAPI.TooltipCreator() {
                @Override
                ...
                public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                    tooltip.addSectionHeading("Ships present in magazine",Alignment.MID,0f);
                    tooltip.showShips(plugin.getCargo().getMothballedShips().getMembersListCopy(),50,true,5f);
                }
            },viewer.getComponentPanel(), TooltipMakerAPI.TooltipLocation.BELOW,false);

        }
        if(!plugin.getCargo().getWeapons().isEmpty()){
            ImageViewer viewer2 = new ImageViewer(20,20,Global.getSettings().getCommoditySpec(Commodities.SHIP_WEAPONS).getIconName());
            ...
            tooltipIncome.addTooltipTo(new TooltipMakerAPI.TooltipCreator() {
                ...
                public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                    ...
                    tooltip.addSectionHeading("Weapons present in magazine",Alignment.MID,0f);
                    tooltip.showCargo(cargo,50,true,5f);
                }
            },viewer2.getComponentPanel(), TooltipMakerAPI.TooltipLocation.BELOW,false);
        }
        if(!plugin.getCargo().getFighters().isEmpty()){
            ImageViewer viewer3 = new ImageViewer(20,20,Global.getSettings().getSpriteName("misc","cargoFighterChip"));
            ...
            tooltipIncome.addTooltipTo(new TooltipMakerAPI.TooltipCreator() {
                ...
                public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                    ...
                    tooltip.addSectionHeading("Fighters present in magazine",Alignment.MID,0f);
                    tooltip.showCargo(cargo,50,true,5f);
                }
            },viewer3.getComponentPanel(), TooltipMakerAPI.TooltipLocation.BELOW,false);
        }
