...
public class AoTDDetailedCommodityUITable extends UITableImpl {
    ...
    public AoTDDetailedCommodityUITable(float width, float height, boolean doesHaveScroller, float xCord, float yCord,boolean isProduction,MarketAPI mainMarket,String commodityId) {
        ...
        if(dropDownButtons.isEmpty()){
            for (MarketAPI marketAPI : Global.getSector().getEconomy().getMarketsCopy()) {
                ...
                } else if (!isProduction&&commodity.getSupplyDemandData().getTotalRawUnitsFromDemand()>0) {
                    AoTDCommodityTableDropDownButton bt = new AoTDCommodityTableDropDownButton(this,width-1,26,0,0,true,commodity,isProduction);
                    dropDownButtons.add(bt);
    ...
    public AoTDDetailedCommodityUITable(float width, float height, boolean doesHaveScroller, float xCord, float yCord,boolean isProduction,FactionAPI faction,String commodityId) {
        super(width, height, doesHaveScroller, xCord, yCord);
        this.isProduction = isProduction;
    ...
    public void advance(float amount) {
        ...
        handleSortButton(colony,
                Comparator.comparing(o -> o.commodity.getMarket().getName()));

        handleSortButton(size,
                Comparator.comparingInt(o -> o.commodity.getMarket().getSize()));
        handleSortButton(acc,
                Comparator.comparingDouble(o -> o.commodity.getMarket().getAccessibilityMod().computeEffective(0f)));
        handleSortButton(faction,Comparator.comparing(x->x.commodity.getMarket().getFaction().getDisplayName()));
        handleSortButton(quantity,Comparator.comparing(x->{
            int supply = x.commodity.getSupplyDemandData().getTotalRawUnitsFromSupply();
            int demand = x.commodity.getSupplyDemandData().getTotalRawUnitsFromDemand();
            if(isProduction){
                return supply;
            }
            return  demand;
        }));
        handleSortButton(excdef,Comparator.comparing(x->{
    ...
    public void createSections() {
        ...
        colony = tooltipOfButtons.addAreaCheckbox("Colony", SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("colony"), 20, 0f);
        size = tooltipOfButtons.addAreaCheckbox("Size",SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("size"), 20, 0f);
        faction = tooltipOfButtons.addAreaCheckbox("Faction",SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("faction"), 20, 0f);
        quantity = tooltipOfButtons.addAreaCheckbox(btName,SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("quantity"), 20, 0f);
        excdef = tooltipOfButtons.addAreaCheckbox(btExcName,SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("excdef"), 20, 0f);
        acc = tooltipOfButtons.addAreaCheckbox("Accessibility",SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("acc"), 20, 0f);
        share = tooltipOfButtons.addAreaCheckbox("Mkt.Share",SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("share"), 20, 0f);
