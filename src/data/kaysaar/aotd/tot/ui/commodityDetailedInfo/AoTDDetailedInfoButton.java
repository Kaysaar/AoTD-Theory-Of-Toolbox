...
public class AoTDDetailedInfoButton extends CustomButton {
    ...
    public AoTDDetailedInfoButton(float width, float height, Object buttonData, float indent, Color base, Color bg, Color bright,boolean showProducers) {
    ...
    public ButtonAPI createButton(TooltipMakerAPI tooltip) {
        return tooltip.addAreaCheckbox("", (Object)null,Color.BLACK, Color.BLACK, Color.BLACK, this.panel.getPosition().getWidth(), this.panel.getPosition().getHeight(), 0.0F, true);
...
//            tooltip.addCustom(panelIndicator,0f).getPosition().inTL((float) StarSystemHoldingTable.widthMap.get("name")*0.75f,centerY-7);
    ...
    public void createContainerContent(CustomPanelAPI container) {
        if(buttonData instanceof AoTDCommodityOnMarket commodity){
            ...
            float textOpad=5f;
           TooltipMakerAPI tlColony = container.createUIElement(AoTDDetailedCommodityUITable.widthMap.get("colony"),height,false);
           ImageViewer viewer =new ImageViewer(height,height,commodity.getMarket().getFaction().getCrest());
           ...
           TooltipMakerAPI tlSize = container.createUIElement(AoTDDetailedCommodityUITable.widthMap.get("size"),height,false);
           tlSize.addPara(market.getSize()+"",market.getFaction().getBaseUIColor(),textOpad).setAlignment(Alignment.MID);
           container.addUIElement(tlSize).inTL(AoTDDetailedCommodityUITable.getStartingX("size"),1);

           TooltipMakerAPI tlFaction = container.createUIElement(AoTDDetailedCommodityUITable.widthMap.get("faction"),height,false);
           tlFaction.addPara(AoTDToolboxMisc.capitalizeFirst(market.getFaction().getDisplayName()),market.getFaction().getBaseUIColor(),textOpad).setAlignment(Alignment.MID);
           container.addUIElement(tlFaction).inTL(AoTDDetailedCommodityUITable.getStartingX("faction"),1);

           TooltipMakerAPI tlQuantity = container.createUIElement(AoTDDetailedCommodityUITable.widthMap.get("quantity"),height,false);
           if(showProducers){
               tlQuantity.addPara(Misc.getWithDGS(commodity.getSupplyDemandData().getTotalRawUnitsFromSupply()),market.getFaction().getBaseUIColor(),textOpad).setAlignment(Alignment.MID);
           }
           else{
               tlQuantity.addPara(Misc.getWithDGS(commodity.getSupplyDemandData().getTotalRawUnitsFromDemand()),market.getFaction().getBaseUIColor(),textOpad).setAlignment(Alignment.MID);
           }
            container.addUIElement(tlQuantity).inTL(AoTDDetailedCommodityUITable.getStartingX("quantity"),1);

           TooltipMakerAPI tlExcDef = container.createUIElement(AoTDDetailedCommodityUITable.widthMap.get("excdef"),height,false);
           if(showProducers){
               String excess = "---";
           ...
           container.addUIElement(tlExcDef).inTL(AoTDDetailedCommodityUITable.getStartingX("excdef"),1);
            TooltipMakerAPI acc = container.createUIElement(AoTDDetailedCommodityUITable.widthMap.get("acc"),height,false);
           ...
           container.addUIElement(tlMarketShare).inTL(AoTDDetailedCommodityUITable.getStartingX("share"),1);
