...
public class AoTDDetailedCommodityPanelContent implements ExtendedUIPanelPlugin {
    ...
    FactionAPI faction;
    public AoTDDetailedCommodityPanelContent(float width, float height, MarketAPI market,String commodityId){
    ...
    public AoTDDetailedCommodityPanelContent(float width, float height, FactionAPI market, String commodityId){
        mainPanel = Global.getSettings().createCustom(width,height,this);
        this.faction = market;
        this.commodity = commodityId;
        this.prevCommodity = commodityId;
    ...
    public void createUI() {
        ...
        TooltipMakerAPI tooltipHeader = contentPanel.createUIElement(AoTDDetailedCommodityUITable.getWidth(),17,false);
        TooltipMakerAPI tooltipHeader2 = contentPanel.createUIElement(AoTDDetailedCommodityUITable.getWidth(),150,false);
        CommoditySpecAPI spec = Global.getSettings().getCommoditySpec(commodity);
        tooltipHeader.addSectionHeading(spec.getName()+" : "+ AoTDToolboxMisc.capitalizeFirst(faction.getDisplayName()),faction.getBaseUIColor(),faction.getDarkUIColor(), Alignment.MID,0f);
        ...
        tooltipHeader2.addCustom(viewer2.getComponentPanel(),5f).getPosition().inTL(AoTDDetailedCommodityUITable.getWidth()-65,40);
        TooltipMakerAPI tooltipGP,tooltipGD,tooltipMV,tooltipFactionProd,tooltipFactionDem,tooltipFactionShare;
        ...
        factionDem = AoTDSectorProductionDemandDataUtils.getTotalDemandFromFaction(commodity,faction.getId());
        factionProd = AoTDSectorProductionDemandDataUtils.getTotalProductionFromFaction(commodity,faction.getId());
        int price = AoTDSectorProductionDemandDataUtils.getPriceAmountTotalAroundSector(commodity,gDem,gProd);

        tooltipMV.setParaFont(Fonts.ORBITRON_12);
        setLabel(tooltipMV.addPara("Global Market Value",faction.getBaseUIColor(), 0f));
        tooltipMV.addTooltipToPrevious(new AoTDGlobalMarketValueData(commodity,faction), TooltipMakerAPI.TooltipLocation.BELOW,false);
        ...
        setLabel(tooltipFactionShare.addPara(
                String.format("%.1f", AoTDSectorProductionDemandDataUtils
                        .getPercentageOfSectorProduction(commodity, factionProd) * 100) + "%",
                Color.ORANGE,
                3f
        ));
        ...
        TooltipMakerAPI tooltipButtons = contentPanel.createUIElement(AoTDDetailedCommodityUITable.getWidth(),20,false);
        producers  = tooltipButtons.addButton("Producers",null,faction.getBaseUIColor(),faction.getDarkUIColor(),Alignment.MID,CutStyle.TOP,150,20,0f);
        producers.getPosition().inTL(0,0);

        consumers  = tooltipButtons.addButton("Consumers",null,faction.getBaseUIColor(),faction.getDarkUIColor(),Alignment.MID,CutStyle.TOP,150,20,0f);
        ...
        if(commodityPanel==null&&market!=null){
            commodityPanel = new AoTDCommodityPanel(AoTDCommodityUITable.getWidth(),contentPanel.getPosition().getHeight(),market,true,this);
        }
        if(tableDem==null){
            tableDem = new AoTDDetailedCommodityUITable(AoTDDetailedCommodityUITable.getWidth(),contentPanel.getPosition().getHeight()-180,true,0,0,false,faction,commodity);
            tableDem.createSections();
            tableDem.createTable();

        }
        if(tableProd==null){
            tableProd = new AoTDDetailedCommodityUITable(AoTDDetailedCommodityUITable.getWidth(),contentPanel.getPosition().getHeight()-180,true,0,0,true,faction,commodity);
        ...
        if(market!=null){
            contentPanel.addComponent(commodityPanel.getMainPanel()).inTL(AoTDDetailedCommodityUITable.getWidth()+18,0);
