...
public class EconomyCommodityData implements ExtendedUIPanelPlugin {
    ...
    public float widthOfSecondSection = 505;
    ...
    public void createUI() {
        ...
        bt = new FactionChooserButton(450, 30, currFactionId, 0f, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Misc.getBrightPlayerColor(),true);
        bt.createUI();
        contentPanel.addComponent(bt.getMainPanel()).inTL(-4, contentPanel.getPosition().getHeight() - 75);

        btGraph = new GraphPeriodChosenButton(250, 30, months, 0f, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Misc.getBrightPlayerColor(),true);
        btGraph.createUI();
        contentPanel.addComponent(btGraph.getMainPanel()).inTL(-4+470, contentPanel.getPosition().getHeight() - 75);

        if (table == null) {
            AoTDCommodityProductionDataTable.resizeToNewWidth(contentPanel.getPosition().getWidth() - widthOfSecondSection - 25);
            table = new AoTDCommodityProductionDataTable(AoTDCommodityProductionDataTable.getWidth(), contentPanel.getPosition().getHeight() - 100, true, 0, 0,currFactionId,months);
    ...
    public void advance(float amount) {
        ...
        if(bt!=null){
            if(bt.mainButton.isChecked()){
                ...
                AshMisc.placePopUpUI(new ChooseFactionPopUp(currFactionId,this),bt.mainButton,470,500);
            }
        }
        if(btGraph!=null){
            if(btGraph.mainButton.isChecked()){
                btGraph.mainButton.setChecked(false);
                AshMisc.placePopUpUI(new ChoosePeriodPopUp(months,this),btGraph.mainButton,250,400);
            }
        }
    }
