...
public class ContractCreatorDetailsPlugin implements ExtendedUIPanelPlugin {
    ...
    public ContractCreatorDetailsPlugin(float width, float height,AoTDTradeContract existingContract) {
        mainPanel = Global.getSettings().createCustom(width,height,this);
    ...
    public void createUI() {
        ...
        if (id != null) {
            ...
            tooltipHeader2.addSectionHeading("Available Commodities To Pick", Alignment.MID, 0f).getPosition().inTL(0,0);
            ...
            createButtonsSection(creatorAPI, buttonWidth, sepX, panelW, heightOfBt, usableWidth, tHeight, listTooltip, sepY, currY,42+height,usableHeight);
    ...
    public void recreateSectionOfCurrentlyPlacedCommoditiesInOrder(float width,float height){
        ...
        for (AoTDTradeContract.TradeContractData value : currentContract.getContractData().values()) {
            CommodityListCustomButton bt = new CommodityListCustomButton(width-15,30,value.getCommodityId(),value.getReqMonthly(),value.getCutFromBasePrice());
            bt.createUI();
        ...
        tlTotal.addPara(Misc.getDGSCredits(Math.abs(currentContract.getPredictedMoneyWorthForMonth())), Color.ORANGE,0f).setAlignment(Alignment.TR);
        ...
        currentlyPickedSection.addUIElement(tlTotal).inTL(0,currentlyPickedSection.getPosition().getHeight()-25);
        currentlyPickedSection.addUIElement(tlTotalLeftSide).inTL(0,currentlyPickedSection.getPosition().getHeight()-25);


        componentPanel.addComponent(currentlyPickedSection).inTL(0, componentPanel.getPosition().getHeight()-currentlyPickedSection.getPosition().getHeight()-5);

    }

    private void createButtonsSection(PlayerContractCreatorAPI creatorAPI, float buttonWidth, float sepX, float panelW, float heightOfBt, float usableWidth, float usableHeight, TooltipMakerAPI listTooltip, float sepY, float currY,float yPosToPlace,float heightToUseForOtherSection) {
        ...
        for (int i = 0; i < commodities.size(); ) {
            ...
            CustomPanelAPI rowPanel =
                    Global.getSettings().createCustom(panelW, heightOfBt, null);
            ...
            for (int c = 0; c < inThisRow; c++) {
                ...
                bigBt.setListener(new CustomButton.ButtonEventListener() {
                    @Override
                    public void onButtonClicked() {
                        currentlyChosen = bigBt.getCommodityId();
                        handleScrollbarSection(panelW, heightToUseForOtherSection, creatorAPI);
                    }
                });
    ...
    private void handleScrollbarSection(float panelW, float usableHeight, PlayerContractCreatorAPI creatorAPI) {
        if(currentlyChosen!=null){
            if(amountSection==null){
                amountSection = new CommodityAmountSection(panelW /2-10, usableHeight,currentlyChosen,currentContract.getMonthlyAmountNeeded(currentlyChosen),creatorAPI.getMaxLimitForCommodityAmount(currentlyChosen), creatorAPI);
                componentPanel.addComponent(amountSection.getMainPanel()).inTL(componentPanel.getPosition().getWidth()-amountSection.getMainPanel().getPosition().getWidth()-5,componentPanel.getPosition().getHeight()-usableHeight-5);
            }
            else{
                amountSection.setCreatorAPI(creatorAPI);
                amountSection.setCommodityId(currentlyChosen,currentContract.getMonthlyAmountNeeded(currentlyChosen),creatorAPI.getMaxLimitForCommodityAmount(currentlyChosen));
    ...
    public void advance(float amount) {
        if(amountSection!=null&& amountSection.isSignalToUpdateAmount()&&id!=null){
            ...
            if(creatorAPI.useUnits()){
                currentContract.addContractData(currentlyChosen,am,creatorAPI.getCutToPayForCommodity(currentlyChosen));

            }
            else{
                float per = amountSection.getCurrSegment()/10f;
                currentContract.addContractData(currentlyChosen,per,creatorAPI.getCutToPayForCommodity(currentlyChosen));

            }
            recreateSectionOfCurrentlyPlacedCommoditiesInOrder(currentlyPickedSection.getPosition().getWidth(),currentlyPickedSection.getPosition().getHeight());
