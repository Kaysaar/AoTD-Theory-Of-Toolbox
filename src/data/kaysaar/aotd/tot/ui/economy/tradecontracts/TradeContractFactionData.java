...
public class TradeContractFactionData implements ExtendedUIPanelPlugin {
    ...
    public void createUI() {
        ...
        float remHeight = contentPanel.getPosition().getHeight()-reputationPanel.getPosition().getHeight()-10;
        float startingY = reputationPanel.getPosition().getHeight()+5;
        if(tradeContractUI==null){
            tradeContractUI = new DetailedTradeContractUI(contentPanel.getPosition().getWidth(),remHeight,null);
    ...
    public void createDetailedContractUI(){
        AoTDTradeContract contract = AoTDTradeContractManager.getInstance().getActiveContracts().get(currentlyChosenContract);
    ...
    public void createReputationPanel() {
        ...
        reputationPanel = Global.getSettings().createCustom(contentPanel.getPosition().getWidth(), 300, null);
        TooltipMakerAPI tl = reputationPanel.createUIElement(reputationPanel.getPosition().getWidth(), reputationPanel.getPosition().getHeight(), false);
        tl.setParaFont(Fonts.ORBITRON_20AA);
        tl.addPara("Merchant Reputation", Misc.getTooltipTitleAndLightHighlightColor(), 0f).setAlignment(Alignment.MID);
        AoTDTradeContractLevelData data =AoTDTradeContractManager.getInstance().getCurrLevelData();
        ProgressBarComponentV2 barComponentV2 = new ProgressBarComponentV2(reputationPanel.getPosition().getWidth() - 5, 25, data.getLevelBandProgress()+" / "+data.getUiThresholdXp(), Fonts.DEFAULT_SMALL, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), data.getProgressLevelFloat());
        ...
        tl.addPara("Level %s - %s", 3f, Color.ORANGE,""+data.getCurrentLevel(),data.getCurrentTitle()).setAlignment(Alignment.MID);
        browseContracts = tl.addButton("Browse posted contracts", null, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Alignment.MID, CutStyle.TL_BR, contentPanel.getPosition().getWidth() - 10, 30, 10);
        ...
        if(!browseContracts.isEnabled()){
            tl.addTooltipToPrevious(new TooltipMakerAPI.TooltipCreator() {
                ...
                @Override
                public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                    tooltip.addPara("Establish your faction by founding a colony before you can accept contracts.",3f);
                }
            }, TooltipMakerAPI.TooltipLocation.RIGHT,false);
        }
        issueContract = tl.addButton("Issue contract", null, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Alignment.MID, CutStyle.BL_TR, contentPanel.getPosition().getWidth() - 10, 30, 10f);
        ...
        if(!issueContract.isEnabled()){
            tl.addTooltipToPrevious(new TooltipMakerAPI.TooltipCreator() {
                ...
                @Override
                public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                    tooltip.addPara("Establish your faction by founding a colony before you can issue contracts.",3f);
                }
            }, TooltipMakerAPI.TooltipLocation.RIGHT,false);
        }
        tl.addSectionHeading("Contract Data",Alignment.MID,5f);
        reputationPanel.getPosition().setSize(reputationPanel.getPosition().getWidth(), tl.getHeightSoFar());
