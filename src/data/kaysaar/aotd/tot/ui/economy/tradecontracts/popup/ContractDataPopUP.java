...
public class ContractDataPopUP extends PopUpUI {
    ...
    public float createUIMockup(CustomPanelAPI panelAPI) {
        mainPanel = Global.getSettings().createCustom(panelAPI.getPosition().getWidth(), panelAPI.getPosition().getHeight(), null);
        tradeContractUI = new DetailedTradeContractUI(panelAPI.getPosition().getWidth(), panelAPI.getPosition().getHeight()-5, contract,true);
    ...
    public void advance(float amount) {
        ...
        if(tradeContractUI!=null){
            if(tradeContractUI.accept.isChecked()){
                ...
                AoTDTradeContractManager.getInstance().getCurrentlyGeneratedInBrowser().remove(contract.getId());
