...
public class TradeContractCustomButton extends CustomButton {
    ...
    public TradeContractCustomButton(float width, float height, Object buttonData, float indent, Color base, Color bg, Color bright,boolean browsingMode) {
...
//            tooltip.addCustom(panelIndicator,0f).getPosition().inTL((float) StarSystemHoldingTable.widthMap.get("name")*0.75f,centerY-7);
    ...
    public void createContainerContent(CustomPanelAPI container) {
        ...
        if(!browsingMode){
            ImageViewer viewer = new ImageViewer(container.getPosition().getHeight()-4,container.getPosition().getHeight()-4, contract.getIconName());
            ...
            float opadText = 17f;
            TooltipMakerAPI contractorNameTooltip = container.createUIElement(widthSection-10-viewer.getComponentPanel().getPosition().getWidth(),container.getPosition().getHeight(),false);
            LabelAPI label = contractorNameTooltip.addPara(contract.getNameOfContract(),contract.getColorOfContractName(),opadText);
            container.addUIElement(contractorNameTooltip).inTL(startingX,0);

            startingX = TradeContractUITable.getStartingX("typeofcontract");
            TooltipMakerAPI tlContractType = container.createUIElement(TradeContractUITable.widthMap.get("typeofcontract"),container.getPosition().getHeight(),false);
            if(contract.getSubTypeOfContractString()!=null){
                tlContractType.addPara(contract.getContractType(),contract.getContractTypeColor(),7f).setAlignment(Alignment.MID);
                tlContractType.addPara(contract.getSubTypeOfContractString(),contract.getContractTypeColor(),2f).setAlignment(Alignment.MID);
            }
            else{
                tlContractType.addPara(contract.getContractType(),contract.getContractTypeColor(),opadText).setAlignment(Alignment.MID);

            }
            container.addUIElement(tlContractType).inTL(startingX,0);

            startingX = TradeContractUITable.getStartingX("commodities");
            AoTDCommodityShortPanelCombined commData = new AoTDCommodityShortPanelCombined(TradeContractUITable.widthMap.get("commodities"),contract.getContractData().size(),contract,true,true);
            container.addComponent(commData.getMainPanel()).inTL(startingX,container.getPosition().getHeight()/2-(commData.getMainPanel().getPosition().getHeight()/2));

            startingX = TradeContractUITable.getStartingX("status");
            TooltipMakerAPI tlDuration = container.createUIElement(TradeContractUITable.widthMap.get("status"),container.getPosition().getHeight(),false);
            ...
            TooltipMakerAPI tlIncome = container.createUIElement(TradeContractUITable.widthMap.get("income"),container.getPosition().getHeight(),false);
            ...
            container.addComponent(DetailedTradeContractUI.createContractorSection(efectiveWidth,50,contract,true)).inTL(2,2);
            ...
            sections.addPara("Duration of contract: %s",3f,Color.ORANGE,contract.getDurationOfContractString());
            sections.addPara("Payment at the end of month: %s",3f,Color.ORANGE,Misc.getDGSCredits(contract.getPredictedMoneyWorthForMonth()));
            ...
            if(contract.getRewards().size()>defaultAmountOfRewards){
                sections.addPara("Contractor includes additional rewards",Misc.getPositiveHighlightColor(),3f);
            }
