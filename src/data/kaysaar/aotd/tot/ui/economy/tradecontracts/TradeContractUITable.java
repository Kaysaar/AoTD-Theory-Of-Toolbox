...
import data.kaysaar.aotd.tot.ui.economy.commoditydata.table.AoTDCommodityProductionDropDownButton;
...
public class TradeContractUITable extends UITableImpl {
    ...
    public TradeContractUITable(float width, float height, boolean doesHaveScroller, float xCord, float yCord) {
        ...
        if(dropDownButtons.isEmpty()){
            for (AoTDTradeContract value : AoTDTradeContractManager.getInstance().getActiveContracts().values()) {
                dropDownButtons.add(new TradeContractDropDownButton(this,width-1,50,0,0, value,isContractBrowsingMode));

            }
        }
    }
    public TradeContractUITable(float width, float height, boolean doesHaveScroller, float xCord, float yCord,boolean isUsingContractBrowsingMode) {
        super(width, height, doesHaveScroller, xCord, yCord);
    ...
    public void createSections() {
        ...
        buttonCommodity = tooltipOfButtons.addAreaCheckbox("Contractor", SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("contractor"), 20, 0f);
        buttonGraph = tooltipOfButtons.addAreaCheckbox("Contract Type", SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("typeofcontract"), 20, 0f);
        buttonSupply = tooltipOfButtons.addAreaCheckbox("Commodities", SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("commodities"), 20, 0f);
        String textStatus = "Status";
        if(isContractBrowsingMode){
            textStatus = "Duration";
        }
        buttonDemand = tooltipOfButtons.addAreaCheckbox(textStatus, SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("status"), 20, 0f);
        buttonNet = tooltipOfButtons.addAreaCheckbox("Monthly Income", SortingState.NON_INITIALIZED, base, bg, bright, widthMap.get("income"), 20, 0f);
    ...
    public void reportButtonPressed(CustomButton buttonPressed) {
        if(buttonPressed instanceof TradeContractCustomButton bt){
            ...
            if(isContractBrowsingMode){
                AshMisc.placePopUpUI(new ContractDataPopUP(bt.getContract(),this),buttonPressed.getPanel(),400,600);

//                AshMisc.placePopUpUIInTL(new ContractDataPopUP(bt.getContract()),buttonPressed.getPanel(),400,600,new Vector2f(-bt.getPanel().getPosition().getWidth(),-bt.getPanel().getPosition().getHeight()));
            }
        }
    }
}
