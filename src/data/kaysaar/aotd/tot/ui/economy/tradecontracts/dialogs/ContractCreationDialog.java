...
import data.kaysaar.aotd.tot.ui.economy.tradecontracts.creator.ContractListTypePlugin;
...
public class ContractCreationDialog extends BasePopUpDialog {
    ...
    public void applyConfirmScript() {
        ...
        if(content.getExplainSection().getCurrentContract()!=null&&content.getExplainSection().getId()!=null) {
           if( !content.getExplainSection().getCurrentContract().getContractData().isEmpty()){
               content.getExplainSection().getCurrentContract().setNewId(Misc.genUID());
               PlayerContractCreatorAPI creatorAPI = AoTDPlayerContractCreatorManager.getCreator(content.getExplainSection().getId());
               creatorAPI.onContractCreated(content.getExplainSection().getCurrentContract());
               AoTDTradeContractManager.getInstance().addContract(content.getExplainSection().getCurrentContract());
