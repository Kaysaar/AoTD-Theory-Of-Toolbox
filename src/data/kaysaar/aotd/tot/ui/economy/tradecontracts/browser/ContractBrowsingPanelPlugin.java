...
public class ContractBrowsingPanelPlugin implements ExtendedUIPanelPlugin {
    ...
    public void createUI() {
        ...
        panelForOffers = Global.getSettings().createCustom(buttonWidth*2+20, mainPanel.getPosition().getHeight(), null);
        if(contractUI==null){
            contractUI = new DetailedTradeContractUI(mainPanel.getPosition().getWidth()-panelForOffers.getPosition().getWidth()-10,mainPanel.getPosition().getHeight()-30,null,true);
        }
        ...
        for (int i = 0; i < contracts.size(); ) {
            ...
            CustomPanelAPI rowPanel = Global.getSettings().createCustom(containerW, buttonHeight, null);

            if (inThisRow == 2) {
                // Left
                AoTDTradeContract c0 = contracts.get(i);
                TradeContractCustomButton bt0 = new TradeContractCustomButton(
                        buttonWidth, buttonHeight, c0, 0f,
                        Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Misc.getBrightPlayerColor(), true
                );
                bt0.createUI();
                bts.add(bt0);
                ...
                TradeContractCustomButton bt1 = new TradeContractCustomButton(
                        buttonWidth, buttonHeight, c1, 0f,
                        Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Misc.getBrightPlayerColor(), true
                );
                bt1.createUI();
                bts.add(bt1);
        ...
        TooltipMakerAPI headerTooltip = panelForOffers.createUIElement(buttonWidth*2+15+additional,20,false);
        ...
        mainPanel.addComponent(contractUI.getMainPanel()).inTL(mainPanel.getPosition().getWidth()-contractUI.getMainPanel().getPosition().getWidth()-5, 0);
    }
    ...
    public void advance(float amount) {
        ...
        if(contractUI!=null&&contractUI.accept!=null&&contractUI.accept.isChecked()){
            ...
            AoTDTradeContract contract = AoTDTradeContractManager.getInstance().getCurrentlyGeneratedInBrowser().remove(currContract);
