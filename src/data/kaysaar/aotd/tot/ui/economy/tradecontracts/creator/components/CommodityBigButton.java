...
public class CommodityBigButton extends CustomButton {
    ...
    public CommodityBigButton(float width, float height, String commodityId) {
        super(width, height, commodityId, 0f, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Misc.getBrightPlayerColor());
    }
    @Override
...
//            tooltip.addCustom(panelIndicator,0f).getPosition().inTL((float) StarSystemHoldingTable.widthMap.get("name")*0.75f,centerY-7);
    ...
    public void createContainerContent(CustomPanelAPI container) {
        ...
        float ratio = 1.75f;
        ImageViewer viewer = new ImageViewer(container.getPosition().getHeight()/ratio,container.getPosition().getHeight()/ratio,spec.getIconName());
        ...
        container.addComponent(viewer.getComponentPanel()).inTL(container.getPosition().getWidth()/2-(viewer.getComponentPanel().getPosition().getWidth()/2),container.getPosition().getHeight()/2-(viewer.getComponentPanel().getPosition().getHeight()/2));
