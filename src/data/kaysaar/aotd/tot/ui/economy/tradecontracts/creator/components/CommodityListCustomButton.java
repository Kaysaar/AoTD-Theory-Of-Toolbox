...
public class CommodityListCustomButton extends CustomButton {
    ...
    public CommodityListCustomButton(float width, float height, String commodityId,int am, float cut) {
        super(width, height, new ButtonData(commodityId,am,cut), 0, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Misc.getBrightPlayerColor());
    }
    @Override
...
//            tooltip.addCustom(panelIndicator,0f).getPosition().inTL((float) StarSystemHoldingTable.widthMap.get("name")*0.75f,centerY-7);
    ...
    public void createContainerContent(CustomPanelAPI container) {
        ...
        float ratio = 1.75f;
        ImageViewer viewer = new ImageViewer(container.getPosition().getHeight()-4,container.getPosition().getHeight()-4,spec.getIconName());
        TooltipMakerAPI tlBottomTitle = container.createUIElement(container.getPosition().getWidth()-4,30,false);
        TooltipMakerAPI amount = container.createUIElement(container.getPosition().getWidth()-4,30,false);
        float opadText = 8f;
        ...
        tlBottomTitle.addPara(Misc.getDGSCredits(am),Color.ORANGE, opadText).setAlignment(Alignment.TR);
        ...
        container.addUIElement(amount).inTL(viewer.getComponentPanel().getPosition().getWidth()+5,0);
    }
}
