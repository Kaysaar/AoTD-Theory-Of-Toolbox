...
public class DetailedTradeContractUI implements ExtendedUIPanelPlugin {
    ...
    public DetailedTradeContractUI(float width, float height, AoTDTradeContract contract, boolean showcaseMode) {
        mainPanel = Global.getSettings().createCustom(width, height, this);
    ...
    public void createUI() {
        ...
        if (contract != null) {
            ...
            TooltipMakerAPI tl = contentPanel.createUIElement(contentPanel.getPosition().getWidth(), contentPanel.getPosition().getHeight() - heightOfButtons-5, true);
            TooltipMakerAPI tlButtons = contentPanel.createUIElement(contentPanel.getPosition().getWidth(), heightOfButtons, false);
            tl.addCustom(createContractorSection(contentPanel.getPosition().getWidth(), 40, contract, false), 0f);
            if (showcaseMode) {
            ...
            if (!contract.isIssuedByPlayer()) {
                if (!showcaseMode) {
                    tl.addPara("Remaining duration of contract : %s", 5f, Color.ORANGE, GraphPeriodChosenButton.getCombinedLabelStringForPeriod(contract.getMonthsRemaining()));
                    tl.addPara("Missed shipments : %s / %s", 3f, Color.ORANGE, contract.getMissedTimes() + "", contract.getAllowedMissedTimes() + "");
                    if (contract.atRiskOfTermination()) {
                        tl.addPara("Missing any portion of a required shipment will immediately terminate the contract, resulting in decline of merchant renown!", Misc.getNegativeHighlightColor(), 3f);
                    }
                } else {
                    tl.addPara("Allowed maximum amount of missed shipments: %s", 5f, Color.ORANGE, contract.getAllowedMissedTimes() + "");
                }
                tl.addPara("*For the contract to count as fulfilled this month, all required cargo must be delivered in full. Any shortfall will count as a missed shipment.", Misc.getGrayColor().brighter(), 3f);

                tl.addSectionHeading("Effects upon successful contract completion", Alignment.MID, 5f);
            } else {
            ...
            if(contract.hasCustomEffectSection()){
                ...
                for (TradeContractRewardDataAPI value : contract.getRewards().values()) {
                    value.createRewardSection(tl, contentPanel.getPosition().getWidth(), TradeContractRewardDataAPI.TradeContractRewardTooltipMode.CONTRACT_DATA, initOpadText);
                }
            ...
            if (!contract.isIssuedByPlayer()) {
                ...
                for (TradeContractRewardDataAPI value : contract.getRewards().values()) {
                    value.createPenaltySectionForNotMeetingContract(tl, contentPanel.getPosition().getWidth(), TradeContractRewardDataAPI.TradeContractRewardTooltipMode.CONTRACT_DATA, initOpadText);
                }
                tl.setBulletedListMode(null);
            }
            if (!showcaseMode) {
                top = tlButtons.addButton("Move to Top Priority", null, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor().darker().darker(), Alignment.MID, CutStyle.TL_BR, contentPanel.getPosition().getWidth() - 15, 25f, 0f);
                CustomPanelAPI row = Global.getSettings().createCustom(contentPanel.getPosition().getWidth(),25,null);
                TooltipMakerAPI rTl = row.createUIElement(row.getPosition().getWidth(),row.getPosition().getHeight(),false);
                freeze = rTl.addButton("Freeze Contract", null, Misc.getBasePlayerColor(),Misc.getDarkPlayerColor(), Alignment.MID, CutStyle.TL_BR, (contentPanel.getPosition().getWidth() - 30)/2, 25f, 0f);
                if(contract.isContractFrozen()){
                    freeze.setText("Resume Contract");
                }
                edit = rTl.addButton("Edit Contract", null, Misc.getBasePlayerColor(), Misc.getPositiveHighlightColor().darker().darker(), Alignment.MID, CutStyle.TL_BR, (contentPanel.getPosition().getWidth() - 30)/2, 25f, 5f);
                freeze.setEnabled(contract.canFreezeContract());
                edit.setEnabled(contract.canEditContract());
                edit.getPosition().inTL(contentPanel.getPosition().getWidth()-10-edit.getPosition().getWidth(),0);
                row.addUIElement(rTl).inTL(-5,0);
                tlButtons.addCustom(row,5f);

                delete = tlButtons.addButton("Terminate Contract", null, Misc.getBasePlayerColor(), Misc.getNegativeHighlightColor().darker().darker(), Alignment.MID, CutStyle.TL_BR, contentPanel.getPosition().getWidth() - 15, 25f, 5f);
                freeze.setEnabled(contract.canFreezeContract());
                delete.setEnabled(contract.canTerminateContract());

                contentPanel.addUIElement(tlButtons).inTL(2, contentPanel.getPosition().getHeight() - 85);
            } else {
                accept = tlButtons.addButton("Accept Contract", null, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor().darker().darker(), Alignment.MID, CutStyle.TL_BR, contentPanel.getPosition().getWidth() - 15, 25f, 0f);
                contentPanel.addUIElement(tlButtons).inTL(2, contentPanel.getPosition().getHeight() - 30);
            }
    ...
    public static CustomPanelAPI createContractorSection(float width, float height, AoTDTradeContract contract, boolean forBrowser) {
        ...
        if (forBrowser) {
            ...
            CustomPanelAPI mainp = new IntrestedInCommodities(width - length - height - 10, 30, Alignment.TL, strs.toArray(new String[0])).getMainPanel();
            ...
            if (contract.getSubTypeOfContractString() == null) {
                ...
                tl.addPara(contract.getContractType() + " - " + contract.getSubTypeOfContractString(), contract.getContractTypeColor(), 3f);
    ...
    public void advance(float amount) {
        ...
        if (delete != null && delete.isChecked()) {
            ...
            ContractTerminationDialog dialog = new ContractTerminationDialog("Terminate Contract", this, contract);
        ...
        if(freeze!=null && freeze.isChecked()){
            ...
            ContractFreezeDialog dialog = new ContractFreezeDialog(contractFreezeText, this, contract);
