...
public class ContractListTypePlugin implements ExtendedUIPanelPlugin {
    ...
    public void createUI() {
        ...
        float opad = 0f;
        for (Map.Entry<String, PlayerContractCreatorAPI> entry : AoTDPlayerContractCreatorManager.contractCreators.entrySet()) {
            ButtonAPI button = tooltipButton.addButton(entry.getValue().getNameOfContract(),entry.getValue().getBaseIdForContract(), Misc.getBasePlayerColor(),Misc.getDarkPlayerColor(),Alignment.MID,CutStyle.NONE, componentPanel.getPosition().getWidth()-9,40,opad);
            int curr = AoTDToolboxMisc.getAmountOfContractsOfSameType(entry.getValue().getBaseIdForContract());
            if(curr>=entry.getValue().getMaxAmountOfConcurrentContracts()&&!entry.getValue().isContractUnlimited()){
                button.setEnabled(false);
            }
            if(!entry.getValue().canUseContract()){
                button.setEnabled(false);
            }
            TooltipMakerAPI.TooltipCreator creator =entry.getValue().generateTooltipCreatorForButtonOnList(400);
            if(creator!=null){
                tooltipButton.addTooltipTo(creator,button, TooltipMakerAPI.TooltipLocation.RIGHT,false);
            }
            buttons.add(button);
            opad =5f;
        }
