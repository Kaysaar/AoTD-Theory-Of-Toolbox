...
public class AoTDDetailedCommodityPanel implements ExtendedUIPanelPlugin {
    ...
    public AoTDDetailedCommodityPanel(float width, float height, String commodityId, String factionId) {
    ...
    public void createUI() {
        ...
        TooltipMakerAPI tl = contentPanel.createUIElement(contentPanel.getPosition().getWidth(), 75, false);
        TooltipMakerAPI tlContent = contentPanel.createUIElement(contentPanel.getPosition().getWidth(), contentPanel.getPosition().getHeight() - 75, false);
        ...
        tl.addCustom(viewer.getComponentPanel(), 5f).getPosition().inTL(contentPanel.getPosition().getWidth() / 2 - (viewer.getComponentPanel().getPosition().getWidth() / 2), 25);
        tlContent.addPara(Global.getSettings().getDescription(spec.getId(), Description.Type.RESOURCE).getText1(), 3f);
        tlContent.addSectionHeading("Estimated Earnings from Trade", colorFaction.getBaseUIColor(), colorFaction.getDarkUIColor(), Alignment.MID, 5f);
        ...
        if (am > 0) {
            if (factionId.equals(Factions.NEUTRAL)) {
                tlContent.addPara("Trade value of entire sector is estimated to be around %s.", 3f, Color.ORANGE, Misc.getDGSCredits(am), spec.getName());

            } else {
                tlContent.addPara(AoTDToolboxMisc.capitalizeFirst(Global.getSector().getFaction(factionId).getDisplayName()) + " earns in total %s from selling %s.", 3f, Color.ORANGE, Misc.getDGSCredits(am), spec.getName());
        ...
        if(!factionId.equals(Factions.NEUTRAL)) {
            checkSectorCurrentEconomyData = tlContent.addButton("Check sector-wide current economy data", null, Misc.getBasePlayerColor(), Misc.getDarkPlayerColor(), Alignment.MID, CutStyle.TL_BR, contentPanel.getPosition().getWidth() - 10f, 25, 10);
            checkSectorCurrentEconomyData.setEnabled(Global.getSector().getIntelManager().isPlayerInRangeOfCommRelay());
            tlContent.addTooltipToPrevious(new TooltipMakerAPI.TooltipCreator() {
                @Override
        ...
        tlContent.addCustom(createTablePanel(contentPanel.getPosition().getWidth(), heightEach, true), 5f);
        tlContent.addCustom(createTablePanel(contentPanel.getPosition().getWidth(), heightEach, false), 5f);
        tlContent.addCustom(createTablePanelContracts(contentPanel.getPosition().getWidth(), 100), 5f);
    ...
    public CustomPanelAPI createTablePanelContracts(float width, float height) {
        ...
        TooltipMakerAPI headerTooltip = mainPanel.createUIElement(mainPanel.getPosition().getWidth(), 20, false);
        FactionAPI colorFaction = Global.getSector().getPlayerFaction();
        headerTooltip.addSectionHeading("Trade Contracts", colorFaction.getBaseUIColor(), colorFaction.getDarkUIColor(), Alignment.MID, 0f);
        mainPanel.addUIElement(headerTooltip).inTL(-5, 0);
        TooltipMakerAPI contentTooltip = mainPanel.createUIElement(mainPanel.getPosition().getWidth(), height - 25, true);
        float initOpad = 1f;
    ...
    public CustomPanelAPI createTablePanel(float width, float height, boolean isProducers) {
        ...
        if (Factions.NEUTRAL.equals(factionId)) {
            if (isProducers) {
                headerTooltip.addSectionHeading("Sector Producers", colorFaction.getBaseUIColor(), colorFaction.getDarkUIColor(), Alignment.MID, 0f);
            } else {
                headerTooltip.addSectionHeading("Sector Consumers", colorFaction.getBaseUIColor(), colorFaction.getDarkUIColor(), Alignment.MID, 0f);
            }
            mainPanel.addUIElement(headerTooltip).inTL(-5, 0);
            TooltipMakerAPI contentTooltip = mainPanel.createUIElement(mainPanel.getPosition().getWidth(), height - 25, true);
            float initOpad = 1f;
            if (isProducers) {
                for (MarketAPI marketAPI : AoTDSectorProductionDemandDataUtils.getFactionMarketsProducers(commodityId, factionId)) {
                    contentTooltip.addCustom(createRow(width, 20, marketAPI, true), initOpad);
                    initOpad = 3f;
                }
            } else {
                for (MarketAPI marketAPI : AoTDSectorProductionDemandDataUtils.getFactionMarketsConsumers(commodityId, factionId)) {
            ...
            if (isProducers) {
                headerTooltip.addSectionHeading("Faction Producers", colorFaction.getBaseUIColor(), colorFaction.getDarkUIColor(), Alignment.MID, 0f);
            } else {
                headerTooltip.addSectionHeading("Faction Consumers", colorFaction.getBaseUIColor(), colorFaction.getDarkUIColor(), Alignment.MID, 0f);
            }
            mainPanel.addUIElement(headerTooltip).inTL(-5, 0);
            TooltipMakerAPI contentTooltip = mainPanel.createUIElement(mainPanel.getPosition().getWidth(), height - 25, true);
            float initOpad = 1f;
    ...
    public CustomPanelAPI createColonySection(float width, float height, MarketAPI market) {
        ...
        ImageViewer viewer = new ImageViewer(height, height, Global.getSector().getFaction(market.getFactionId()).getCrest());
        section.addComponent(viewer.getComponentPanel()).inTL(0, 0);
        LabelAPI label = headerTooltip.addPara(market.getName() + " ( size " + market.getSize() + " )", market.getFaction().getBaseUIColor(), 0f);
    ...
    public CustomPanelAPI createRow(float width, float height, MarketAPI market, boolean production) {
        CustomPanelAPI section = Global.getSettings().createCustom(width, height, null);
        ...
        if (production) {
            ...
            label = headerTooltip.addPara(Misc.getWithDGS(marketCom.getSupplyDemandData().getTotalRawUnitsFromDemand()), Color.ORANGE, 0f);
        }
        label.getPosition().inTL(labelWidth / 2 - (label.computeTextWidth(label.getText())), height / 2 - (label.computeTextHeight(label.getText()) / 2));
    ...
    public CustomPanelAPI createRow(float width, float height, AoTDTradeContract contract) {
        ...
        float labelWidth = width * 0.3f;
        LabelAPI label = null;
        label = headerTooltip.addPara(Misc.getWithDGS(contract.getContractData().get(commodityId).getReqMonthly()), Color.ORANGE, 0f);
        label.getPosition().inTL(labelWidth / 2 - (label.computeTextWidth(label.getText())), height / 2 - (label.computeTextHeight(label.getText()) / 2));
        section.addComponent(colonySection).inTL(0, 0);
        section.addUIElement(headerTooltip).inTL(width * 0.7f + 5, 0);
        return section;
    ...
    public void advance(float amount) {
        if (checkSectorCurrentEconomyData != null && checkSectorCurrentEconomyData.isChecked()) {
            ...
            AshMisc.initPopUpDialog(new CommodityDetailDialog(Global.getSector().getFaction(factionId), commodityId), AoTDDetailedCommodityUITable.getWidth() + 38, 665);
        }
    }
