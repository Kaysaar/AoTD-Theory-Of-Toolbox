...
public class FactionChooserButton extends CustomButton {
    public FactionChooserButton(float width, float height, String factionId, float indent, Color base, Color bg, Color bright,boolean withArrow) {
    ...
    public void createButtonContent(TooltipMakerAPI tooltip) {
        ...
        if(isWithArrow){
            ...
            container.addComponent(panelIndicator).inTL(container.getPosition().getWidth()-20,centerY-7);
    ...
    public void createContainerContent(CustomPanelAPI container) {
        ...
        TooltipMakerAPI tlContainer = container.createUIElement(container.getPosition().getWidth()-iconSize-15,container.getPosition().getHeight(),false);
        ImageViewer viewer = new ImageViewer(iconSize,iconSize,Global.getSector().getFaction(getFactionId()).getCrest());
        ...
        if(Factions.NEUTRAL.equals(getFactionId())){
            ...
            LabelAPI label = tlContainer.addPara(AoTDToolboxMisc.capitalizeFirst(Global.getSector().getFaction(getFactionId()).getDisplayName()),0f);
            label.getPosition().inTL(0,container.getPosition().getHeight()/2-(label.computeTextHeight(label.getText())/2));
        }
