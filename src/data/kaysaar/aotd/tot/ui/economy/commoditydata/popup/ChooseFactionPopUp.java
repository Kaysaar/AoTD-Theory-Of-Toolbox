...
public class ChooseFactionPopUp extends PopUpUI {
    ...
    public float createUIMockup(CustomPanelAPI panelAPI) {
        ...
        TooltipMakerAPI tooltipMakerAPI = mainPanel.createUIElement(mainPanel.getPosition().getWidth(),mainPanel.getPosition().getHeight(),true);
        FactionChooserButton chosenPlayer = new FactionChooserButton(panelAPI.getPosition().getWidth()-20,30, Factions.PLAYER,0f, Misc.getBasePlayerColor(),Misc.getDarkPlayerColor(),Misc.getBrightPlayerColor(),false);
        if(factions.isEmpty()){
            factions.add(new FactionChooserButton(panelAPI.getPosition().getWidth()-20,30,Factions.NEUTRAL,0f,Misc.getBasePlayerColor(),Misc.getDarkPlayerColor(),Misc.getBrightPlayerColor(),false));
            ...
            for (String s : AoTDTradeManager.getInstance().getAllFactionTradeData().keySet()) {
                ...
                factions.add(new FactionChooserButton(panelAPI.getPosition().getWidth()-20,30,s,0f,Misc.getBasePlayerColor(),Misc.getDarkPlayerColor(),Misc.getBrightPlayerColor(),false));
            }
        }
