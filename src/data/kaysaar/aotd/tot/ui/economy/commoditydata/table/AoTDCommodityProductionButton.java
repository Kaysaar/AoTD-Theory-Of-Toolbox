...
public class AoTDCommodityProductionButton extends CustomButton {
    public static class AoTDCommodityProductionButtonData{
        ...
        public AoTDCommodityProductionButtonData(String commodityId, String factionId,boolean showAll) {
            this.commodityId = commodityId;
            this.factionId = factionId;
            showAll = true;
        }
    ...
    public AoTDCommodityProductionButton(float width, float height, AoTDCommodityProductionButtonData buttonData, float indent, Color base, Color bg, Color bright) {
...
//            tooltip.addCustom(panelIndicator,0f).getPosition().inTL((float) StarSystemHoldingTable.widthMap.get("name")*0.75f,centerY-7);
    ...
    public void createContainerContent(CustomPanelAPI container) {
        ...
        TooltipMakerAPI tl = container.createUIElement(widthOfCom-iconSize-10,container.getPosition().getHeight(),false);
        ...
        if(data.showAll){
            component = new CommodityGraphComponent(AoTDCommodityProductionDataTable.widthMap.get("graph"),40, data.commodityId,data.factionId,Integer.MAX_VALUE);

        }
        else{
            component = new CommodityGraphComponent(AoTDCommodityProductionDataTable.widthMap.get("graph"),40, data.commodityId,data.factionId,data.months);
        }
        container.addComponent(component.getMainPanel()).inTL(AoTDCommodityProductionDataTable.getStartingX("graph"),5);
