...
public class GrandWonderButtonComponent extends CustomButton {
    public GrandWonderButtonComponent(float width, float height, GrandWonderAPI buttonData, MarketAPI market) {
        super(width, height, buttonData, 0f, market.getFaction().getBaseUIColor(),market.getFaction().getDarkUIColor(), market.getFaction().getBrightUIColor());
        isWithArrow = false;
    }
...
//            tooltip.addCustom(panelIndicator,0f).getPosition().inTL((float) StarSystemHoldingTable.widthMap.get("name")*0.75f,centerY-7);
    ...
    public void createContainerContent(CustomPanelAPI container) {
        ...
        container.addComponent(viewer.getComponentPanel()).inTL(2,container.getPosition().getHeight()/2-25);
        TooltipMakerAPI tlLabels = container.createUIElement(container.getPosition().getWidth()-105,container.getPosition().getHeight(),false);
        tlLabels.addPara(getButtonData().getCurrentName(),2f);
        GrandWonderTypeSpecAPI typeSpecAPI =GrandWonderTypeManager.getSpec(getButtonData().getWonderTypeId());
