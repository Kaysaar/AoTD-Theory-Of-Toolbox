...
public class IncomePanel implements ExtendedUIPanelPlugin {
    ...
    public void createUI() {
        ...
        title = tooltip.addPara("Estimated Monthly Income", tiedMarket.getFaction().getBaseUIColor(), 4f);
        ...
        contentPanel.getPosition().setSize(Math.max(title.computeTextWidth(title.getText()), number.computeTextWidth(number.getText())),tooltip.getHeightSoFar());
        tooltip.addTooltipTo(new IncomePanelOnHover(tiedMarket),contentPanel, TooltipMakerAPI.TooltipLocation.BELOW,false);
