...
//		supply(1, Commodities.ORGANICS, -deficit, getDeficitText(Commodities.HEAVY_MACHINERY));
        ...
        if(deficit.two>0){
            float percentage = (float) deficit.two /this.getDemand(Commodities.HEAVY_MACHINERY).getQuantity().getModifiedInt();
            int reduction = Math.round(this.getSupply(Commodities.FOOD).getQuantity().getModifiedInt()*percentage);
