...
//	}
        ...
        getDemand(Commodities.FOOD).getQuantity().modifyMultAlways("aotd_new_scale", (float) Math.pow(2,market.getSize()-1),"");
        ...
        getDemand(Commodities.DOMESTIC_GOODS).getQuantity().modifyMultAlways("aotd_new_scale", (float) Math.pow(2,market.getSize()-3),"");
        ...
        if (deficit.two > 0) {
            market.getStability().modifyFlat("aotd_shortage_food_organics", -deficit.two, getDeficitText(deficit.one));
        } else {
...
//		}
        ...
        market.getStats().getDynamic().getMod(AoTDMarketStats.AOTD_GRAND_WONDER_COUNT).modifyFlat(getModId(), Math.max(Math.floorDiv(Misc.getMaxIndustries(market),2),1), "Grand Wonder Slots");
        int overLimit = (int) (GrandWonderManager.getAmountOfWonders(market) - market.getStats().getDynamic().getMod(AoTDMarketStats.AOTD_GRAND_WONDER_COUNT).computeEffective(0f));
        if(overLimit>0){

            market.getUpkeepMult().modifyMult("aotd_wonder_pen",overLimit+1,"Grand wonder limit");
        }
    }
    ...
    protected void addPostDemandSection(TooltipMakerAPI tooltip, boolean hasDemand, IndustryTooltipMode mode) {
        if (mode != IndustryTooltipMode.NORMAL || isFunctional()) {
            ...
            for (MutableStat.StatMod mod : market.getStability().getFlatMods().values()) {
                if (mod.source.startsWith(getModId())||mod.source.equals("aotd_shortage_food_organics")) {
