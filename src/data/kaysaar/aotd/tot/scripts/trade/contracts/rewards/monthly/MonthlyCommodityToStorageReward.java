...
public class MonthlyCommodityToStorageReward implements TradeContractRewardDataAPI {
    ...
    public void createRewardSection(TooltipMakerAPI tooltip, float width, TradeContractRewardTooltipMode mode, float defaultOpadText) {
        MarketAPI gatheringPoint = Global.getSector()
                .getPlayerFaction()
                .getProduction()
                .getGatheringPoint();
    ...
    public void executeRewardMonthly(int amountOfCommoditiesDeliveredThisMonth, int reqThisMonth, String commodityId) {
        ...
        if (gatheringPoint != null) {
            gatheringPoint.getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addCommodity(commodityId,amountOfCommoditiesDeliveredThisMonth);
        }
    }
}
