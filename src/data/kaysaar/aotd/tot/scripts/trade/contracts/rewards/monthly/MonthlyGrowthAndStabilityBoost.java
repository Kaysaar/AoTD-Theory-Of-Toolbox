...
public class MonthlyGrowthAndStabilityBoost implements TradeContractRewardDataAPI {
    ...
    @Override
    public void createPenaltySectionForNotMeetingContract(TooltipMakerAPI tooltip,
                                                          float width, TradeContractRewardTooltipMode mode, float defaultOpadText) {
    ...
    public void executeRewardMonthly(int amountOfCommoditiesDeliveredThisMonth, int reqThisMonth, String commodityId) {
        int demand = AoTDSectorProductionDemandDataUtils.getTotalDemandFromFactionIgnoreContracts(commodityId, Factions.PLAYER);
        ...
        if (commodityId.equals(Commodities.FOOD)&&fulfilledDemandCycles>=1) {
            ...
            for (MarketAPI marketAPI : AshMisc.getMarketsUnderPlayer()) {
                marketAPI.getStability().modifyFlat("aotd_contract_civ",stab, tiedContract.getSubTypeOfContractString());
            }
        }
        if (commodityId.equals(Commodities.DOMESTIC_GOODS)) {
            for (MarketAPI marketAPI : AshMisc.getMarketsUnderPlayer()) {
                AoTDToolboxCivContractPopBoost.getPluginInstance(marketAPI).setCurrWeight(Math.round(fulfilledDemandCycles*CivilianSupplyProgram.growthPerMetDemandDomesticGoods));
            }
        }
    }
}
