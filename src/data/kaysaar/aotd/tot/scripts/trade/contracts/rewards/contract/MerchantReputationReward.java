...
public class MerchantReputationReward implements TradeContractRewardDataAPI {
    ...
    public void createRewardSection(TooltipMakerAPI tooltip, float width, TradeContractRewardTooltipMode mode,float defaultOpadText) {
        tooltip.addPara("Increase merchant renown by %s",defaultOpadText, Color.ORANGE,""+amountXPIncrease);
    }
    ...
    public void createPenaltySectionForNotMeetingContract(TooltipMakerAPI tooltip, float width, TradeContractRewardTooltipMode mode, float defaultOpadText) {
        tooltip.addPara("Decrease merchant renown by %s",defaultOpadText, Misc.getNegativeHighlightColor(),""+Math.round(amountXPIncrease/2f));
    ...
    public void executePenaltyAthTheTerminationOfContract(boolean wasTerminatedByPlayerManually) {
        AoTDTradeContractManager.getInstance().getCurrLevelData().removeXp(Math.round(amountXPIncrease/2f));
    }
