...
public class AICoreReward implements TradeContractRewardDataAPI {
    ...
    public AICoreReward() {
        ...
        float alphaW = validAICoresMapWeight.get(Commodities.ALPHA_CORE) * (1f + 2.5f * t);  // up to ~3.5x
        float betaW  = validAICoresMapWeight.get(Commodities.BETA_CORE)  * (1f + 1.0f * t);  // up to ~2x
        float gammaW = validAICoresMapWeight.get(Commodities.GAMMA_CORE) * (1f - 0.35f * t); // down to 65%
        ...
        if (level >= 15 && r.nextFloat() < 0.10f) {
            aiCoresToGive.put(Commodities.ALPHA_CORE, aiCoresToGive.getOrDefault(Commodities.ALPHA_CORE, 0) + 1);
        }
    }
    ...
    public void createRewardSection(TooltipMakerAPI tooltip, float width, TradeContractRewardTooltipMode mode, float defaultOpadText) {
        for (Map.Entry<String, Integer> entry : aiCoresToGive.entrySet()) {
            tooltip.addPara("Gain %s %s", defaultOpadText, Color.ORANGE,
                    entry.getValue() + "x " , Global.getSettings().getCommoditySpec(entry.getKey()).getName());
        }
    }
    ...
    public void executeRewardAtTheEndOfContract() {
        ...
        if (market != null) {
            for (Map.Entry<String, Integer> e : aiCoresToGive.entrySet()) {
                market.getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addCommodity(e.getKey(), e.getValue());
            }
        }
    }
