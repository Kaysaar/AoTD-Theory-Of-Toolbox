...
public class PlayerIssuedSupplyContract implements PlayerContractCreatorAPI {
    ...
    public LinkedHashSet<String> getAvailableCommoditiesForContract() {
        LinkedHashSet<String> available = new LinkedHashSet<>(AoTDTradeManager.getInstance().getPossibleCommoditiesDemandedOrSupplied());
    ...
    public int getMaxLimitForCommodityAmount(String commodityId) {
        int totalProd = AoTDSectorProductionDemandDataUtils.getTotalProductionFromFaction(commodityId, Factions.PLAYER);
    ...
    public void createContractExplanationSection(TooltipMakerAPI tooltip, float width) {
        ...
        if(fullProdCommodities.isEmpty()){
            ...
            LabelAPI labelAPI = tooltip.addPara(
    ...
    @Override
    public void createProcTooltipSection(TooltipMakerAPI tooltip, float width,float price , int am, String commodity) {
        tooltip.addPara("Procurement of this commodity will cost you : %s", 3f, Color.ORANGE, Misc.getDGSCredits(price));
    }
