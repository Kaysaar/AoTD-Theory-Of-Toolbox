...
public class BlueprintReward implements TradeContractRewardDataAPI {
    ...
    public void createRewardSection(TooltipMakerAPI tooltip, float width, TradeContractRewardTooltipMode mode, float defaultOpadText) {
        if(blueprintData==BlueprintData.FIGHTER){
            ...
            tooltip.addPara("Gain blueprint for %s",defaultOpadText, Color.ORANGE,spec.getWingName());
        }
        else if(blueprintData==BlueprintData.WEAPON){
            WeaponSpecAPI spec = Global.getSettings().getWeaponSpec(id);
            tooltip.addPara("Gain blueprint for %s",defaultOpadText, Color.ORANGE,spec.getWeaponName());

        }
        else if(blueprintData==BlueprintData.SHIP){
            ShipHullSpecAPI spec = Global.getSettings().getHullSpec(id);
            tooltip.addPara("Gain blueprint for %s",defaultOpadText, Color.ORANGE,spec.getHullNameWithDashClass());


        }
        else if(blueprintData==BlueprintData.HULLSPEC){
            HullModSpecAPI spec = Global.getSettings().getHullModSpec(id);
            tooltip.addPara("Gain blueprint for %s",defaultOpadText, Color.ORANGE,spec.getDisplayName());
    ...
    public void createRewardForCustom(TooltipMakerAPI tooltip, float width,TradeContractRewardTooltipMode mode, float defaultOpadText) {
