...
public class MonthlyFuelAccessibilityBoost implements TradeContractRewardDataAPI {
    ...
    public void createRewardSection(TooltipMakerAPI tooltip, float width, TradeContractRewardTooltipMode mode, float defaultOpadText) {
        float percent = tiedContract.getMonthlyAmountNeeded(Commodities.FUEL)/ FuelLogisticProgram.perFuel;
        tooltip.addPara("Increases accessibility across all colonies by %s.", 3f, Color.ORANGE, Misc.getRoundedValueMaxOneAfterDecimal(percent) + "%");
        tooltip.addPara("*This bonus is applied while the contract is fully maintained. If conditions are not met, the effect is reduced.*", Misc.getGrayColor(), 3f);
    }
    ...
    public void executeRewardMonthly(int amountOfCommoditiesDeliveredThisMonth, int reqThisMonth, String commodityId) {
        if(commodityId.equals(Commodities.FUEL)){
            float percentage = tiedContract.getMonthlyAmountNeeded(Commodities.FUEL)/FuelLogisticProgram.perFuel;
            percentage/=100;
            for (MarketAPI marketAPI : AshMisc.getMarketsUnderPlayer()) {
                marketAPI.getAccessibilityMod().modifyFlat("aotd_fuel_contract",percentage,tiedContract.getSubTypeOfContractString());
            }
        }
    }
}
