...
public class MonthlyDefenceBoost implements TradeContractRewardDataAPI {
    ...
    @Override
    public void createRewardSection(TooltipMakerAPI tooltip, float width, TradeContractRewardTooltipMode mode,
                                    float defaultOpadText) {
        float fleetPercent = tiedContract.getMonthlyAmountNeeded(Commodities.SHIPS) / StrategicArmamentSupply.shipsPerFleetSize;
        float defensePercent = tiedContract.getMonthlyAmountNeeded(Commodities.HAND_WEAPONS) / StrategicArmamentSupply.gunsPerDefense;

        tooltip.addPara(
                "Increases fleet size across all colonies by %s.",
                3f,
                Color.ORANGE,
                Misc.getRoundedValueMaxOneAfterDecimal(fleetPercent) + "%"
        );
        ...
        tooltip.addPara(
                ...
                3f,
                Color.ORANGE,
                Misc.getRoundedValueMaxOneAfterDecimal(defensePercent) + "%"
        );
    ...
    @Override
    public void createPenaltySectionForNotMeetingContract(TooltipMakerAPI tooltip, float width,
                                                          TradeContractRewardTooltipMode mode, float defaultOpadText) {
    ...
    public void executePenaltyAthTheTerminationOfContract(boolean wasTerminatedByPlayerManually) {
        for (MarketAPI marketAPI : AshMisc.getMarketsUnderPlayer()) {
            marketAPI.getStats().getDynamic().getMod(Stats.COMBAT_FLEET_SIZE_MULT).unmodifyFlat(FLEET_MOD_ID);
            marketAPI.getStats().getDynamic().getMod(Stats.GROUND_DEFENSES_MOD).unmodifyMult(DEFENSE_MOD_ID);
        }
    }
    ...
    public void executeRewardMonthly(int amountOfCommoditiesDeliveredThisMonth, int reqThisMonth, String commodityId) {
        if (Commodities.SHIPS.equals(commodityId)) {
            float percentage = amountOfCommoditiesDeliveredThisMonth / StrategicArmamentSupply.shipsPerFleetSize;
            percentage /= 100f;

            for (MarketAPI marketAPI : AshMisc.getMarketsUnderPlayer()) {
                marketAPI.getStats().getDynamic().getMod(Stats.COMBAT_FLEET_SIZE_MULT)
                        .modifyFlat(FLEET_MOD_ID, percentage, tiedContract.getSubTypeOfContractString());
            }
        }

        if (Commodities.HAND_WEAPONS.equals(commodityId)) {
            float percentage = amountOfCommoditiesDeliveredThisMonth / StrategicArmamentSupply.gunsPerDefense;
            percentage /= 100f;

            for (MarketAPI marketAPI : AshMisc.getMarketsUnderPlayer()) {
                marketAPI.getStats().getDynamic().getMod(Stats.GROUND_DEFENSES_MOD)
                        .modifyMult(DEFENSE_MOD_ID, 1+percentage, tiedContract.getSubTypeOfContractString());
            }
        }
    }
}