...
public class GrandWonderManager {
    ...
    public static boolean canBuiltAdditionalWonderDueToSlots(MarketAPI market){
        int am = (int) market.getStats().getDynamic().getMod(AoTDMarketStats.AOTD_GRAND_WONDER_COUNT).computeEffective(0f);
    ...
    public boolean hadExceededLimitOfThisWonder(String id, String wonderType, MarketAPI marketAPI){
        return getBuiltSoFar(id)<GrandWonderTypeManager.getSpec(wonderType).getMaxAmountOfWonderOfType(id,marketAPI);
    }
    ...
    public static GrandWonderManager getInstance(){
        Global.getSector().getPersistentData().computeIfAbsent(memKey, k -> new GrandWonderManager());
