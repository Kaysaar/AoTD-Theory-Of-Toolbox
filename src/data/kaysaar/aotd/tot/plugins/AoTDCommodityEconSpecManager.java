...
public class AoTDCommodityEconSpecManager {
    ...
    public static LinkedHashSet<String>possibleCommoditiesProducedByIndustry = new LinkedHashSet<>();
    ...
    public static void loadSpecs(){
        ...
        try {
            array = Global.getSettings().getMergedSpreadsheetDataForMod("commodityId",specsFilename,"aotd_theory_of_toolbox");
    ...
    public static int getCargoAmountFromSupplyOrDemand(int units,boolean isDemand,String commodityId){
        MutableStat stat = new MutableStat(units);
        if(isDemand){
            return getEconSpec(commodityId).getCalculationScript().getRawUnitsFromDemand(stat,null,commodityId,null);
        }
        else{
            return getEconSpec(commodityId).getCalculationScript().getRawUnitsFromSupply(stat,null,commodityId,null);
        }
    }
    ...
    public static AoTDCommodityEconSpec getEconSpec(String commodityId){
        return specs.getOrDefault(commodityId,new AoTDCommodityEconSpec(commodityId,1,1,AoTDBaseDemSupCalc.class.getName(),0.05f,0.15f));
    }
