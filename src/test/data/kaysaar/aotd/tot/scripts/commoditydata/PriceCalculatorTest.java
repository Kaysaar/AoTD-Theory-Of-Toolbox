...
import data.kaysaar.aotd.tot.scripts.commoditydata.PriceCalculator;
import data.kaysaar.aotd.tot.scripts.commoditydata.PriceCalculator.TransactionDirection;

import org.junit.jupiter.api.Test;
import static org.junit.jupiter.api.Assertions.*;
import static data.kaysaar.aotd.tot.scripts.commoditydata.PriceCalculator.*;
...
public class PriceCalculatorTest {
    ...
    public void sellToEntity_MoreUnits_LowerAveragePrice() {
        ...
        final float avg1 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, 10, stored, BASE_PRICE, DEMAND);
        final float avg2 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, 20, stored, BASE_PRICE, DEMAND);
    ...
    public void buyFromEntity_MoreUnits_HigherAveragePrice() {
        ...
        final float avg1 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 10, stored, BASE_PRICE, DEMAND);
        final float avg2 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 20, stored, BASE_PRICE, DEMAND);
    ...
    public void buySplit_SameTotalCost() {
        ...
        final float avgOnce = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, totalAmount, stored, BASE_PRICE, DEMAND);
        final double totalOnce = avgOnce * totalAmount;

        // Buy in two halves, updating stock after each
        final long half = totalAmount / 2;
        final float avgFirst = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, half, stored, BASE_PRICE, DEMAND);
        final double costFirst = avgFirst * half;
        final double newStock = stored - half;

        final float avgSecond = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, half, newStock, BASE_PRICE, DEMAND);
    ...
    public void sellSplit_SameTotalRevenue() {
        ...
        final float avgOnce = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, totalAmount, stored, BASE_PRICE, DEMAND);
        final double totalOnce = avgOnce * totalAmount;

        final long half = totalAmount / 2;
        final float avgFirst = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, half, stored, BASE_PRICE, DEMAND);
        final double revFirst = avgFirst * half;
        final double newStock = stored + half;

        final float avgSecond = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, half, newStock, BASE_PRICE, DEMAND);
    ...
    public void buyMoreThanStock_HigherAverageThanExactStock() {
        ...
        final float avgExact = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, exactAmount, stored, BASE_PRICE, DEMAND);
        final float avgOver = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, overAmount, stored, BASE_PRICE, DEMAND);
    ...
    public void buyWhenStockNegative_PriceHigh() {
        ...
        final float avg = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 10, stored, BASE_PRICE, DEMAND);
    ...
    public void neutralPrice_ReturnsInstantaneousPrice() {
        ...
        final float neutral = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, stored, BASE_PRICE, DEMAND);
        final float buy1 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 0, stored, BASE_PRICE, DEMAND);
    ...
    public void sellWhenStockNegative_GivesVeryHighPrice() {
        ...
        final float avg = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, 20, stored, BASE_PRICE, DEMAND);
    ...
    public void priceNeverBelowOne() {
        ...
        final float avgBuy = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 1, stored, BASE_PRICE, DEMAND);
        final float avgSell = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, 1, stored, BASE_PRICE, DEMAND);
        final float neutral = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, stored, BASE_PRICE, DEMAND);
    ...
    public void zeroAmount_ReturnsInstantaneousPrice() {
        ...
        final float p1 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, 0, stored, BASE_PRICE, DEMAND);
        final float p2 = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, stored, BASE_PRICE, DEMAND);
    ...
    public void tradeAcrossZoneBoundaries_StillAdditive() {
        ...
        final float avg1 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 5, stored, BASE_PRICE, DEMAND);
        final double cost1 = avg1 * 5;
        final double newStock = stored - 5;
        final float avg2 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 15, newStock, BASE_PRICE, DEMAND);
    ...
    public void extremelyNegativeStock_DoesNotOverflow() {
        ...
        assertTrue(avg >= BASE_PRICE * PriceCalculator.PRICE_MULT_CEILING, "Extremely negative stock should give extremely high price");
    }
    ...
    public void sellThenBuy_NetLoss() {
        ...
        final float avgSell = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, amount, stored, BASE_PRICE, DEMAND);
        final double revenue = avgSell * amount;
        final double stockAfterSell = stored + amount;

        final float avgBuy = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, amount, stockAfterSell, BASE_PRICE, DEMAND);
    ...
    public void buyThenSell_NetLoss() {
        ...
        final float avgBuy = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, amount, stored, BASE_PRICE, DEMAND);
        final double cost = avgBuy * amount;
        final double stockAfterBuy = stored - amount;

        final float avgSell = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, amount, stockAfterBuy, BASE_PRICE, DEMAND);
        final double revenue = avgSell * amount;
    ...
    public void instantaneousPriceDecreasesWithStock() {
        ...
        for (int i = 0; i < stocks.length - 1; i++) {
            final float p1 = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, stocks[i],   BASE_PRICE, DEMAND);
            final float p2 = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, stocks[i+1], BASE_PRICE, DEMAND);
    ...
    public void tinyBuyInDeepDeficitHitsCeiling() {
        ...
        final float avg = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 1, stored, BASE_PRICE, DEMAND);
        assertEquals(BASE_PRICE * PriceCalculator.PRICE_MULT_CEILING, avg, 0.01);
    }
    ...
    public void hugeAmountDoesNotOverflow() {
        ...
        final float avg = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, huge, stored, BASE_PRICE, DEMAND);
    ...
    public void massiveExcessHitsFloor() {
        ...
        final float avg = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, 1, stored, BASE_PRICE, DEMAND);
        assertTrue(avg >= BASE_PRICE * PriceCalculator.PRICE_MULT_FLOOR);
    }
    ...
    public void demandBelowInherentFloorIsIgnored() {
        ...
        final float p = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, INHERENT_DEMAND, BASE_PRICE, tinyDemand);
    ...
    public void stockZeroIsValid() {
        final float avg = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, 0, BASE_PRICE, DEMAND);
    ...
    public void stockDoubleMaxValueDoesNotOverflow() {
        ...
        final float avg = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, stored, BASE_PRICE, DEMAND);
        assertTrue(avg >= BASE_PRICE * PriceCalculator.PRICE_MULT_FLOOR);
    }
    ...
    public void entityBuyingIncreasesStock() {
        ...
        PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_BUYING, amount, stored, BASE_PRICE, DEMAND);
    ...
    public void entitySellingDecreasesStock() {
        ...
        PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, amount, stored, BASE_PRICE, DEMAND);
    ...
    public void neutralIgnoresAmount() {
        final double stored = 50;
        final float p1 = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 0, stored, BASE_PRICE, DEMAND);
        final float p2 = PriceCalculator.getUnitPrice(TransactionDirection.NEUTRAL, 100, stored, BASE_PRICE, DEMAND);
    ...
    public void threeWaySplitSameTotalCost() {
        ...
        final float avgOnce = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, total, stored, BASE_PRICE, DEMAND);
        final double costOnce = avgOnce * total;

        double s = stored;
        final float a1 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, p1, s, BASE_PRICE, DEMAND);
        final double c1 = a1 * p1;
        s -= p1;
        final float a2 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, p2, s, BASE_PRICE, DEMAND);
        final double c2 = a2 * p2;
        s -= p2;
        final float a3 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, p3, s, BASE_PRICE, DEMAND);
    ...
    public void crossAllThreeZonesAdditivity() {
        ...
        final float avgOnce = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, buy, stored, BASE_PRICE, DEMAND);
        final double costOnce = avgOnce * buy;

        // split: first half, then second half
        final long half = buy / 2;
        final float a1 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, half, stored, BASE_PRICE, DEMAND);
        final double c1 = a1 * half;
        final double afterHalf = stored - half;
        final float a2 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, half, afterHalf, BASE_PRICE, DEMAND);
    ...
    public void priceScalesLinearlyWithBasePrice() {
        ...
        final float p1 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, amount, stored, bp1, DEMAND);
        final float p2 = PriceCalculator.getUnitPrice(TransactionDirection.ENTITY_SELLING, amount, stored, bp2, DEMAND);
